...
public class PermissionController implements ApiControllerSupport {
    ...
    public ApiResponse<PageResponse<PermissionVO>> list() {
        List<PermissionVO> permissions = service.list();
        return ok(PageResponse.of(permissions, 1, permissions.size(), permissions.size()));
    }
}
